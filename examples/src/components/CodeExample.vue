...
onMounted(async () => {
  try {
    ...
    const modules = import.meta.glob('/code/**/*.{vue,ts,tsx,js,jsx}', {
      query: '?raw',
      import: 'default'
    })

    // Normalize the path - remove /examples prefix since we're in examples/ root
    let normalizedPath = props.examplePath.startsWith('/')
      ? props.examplePath
      : `/${props.examplePath}`
    normalizedPath = normalizedPath.replace(/^\/examples/, '')
